...
class FormDangKy extends Component {
  ...
  handelInput = (e) => {
    ...
    if (attrValue === 'maSV') {
      ...
      if (!reg.test(value)) {
        message = name + ' không đúng định dạng (mã sinh viên chỉ bao gồm kí tự thường và số)'
      }
    }
    ...
    if (attrValue === 'hoTen') {
      ...
      // ktr ko đúng định dạng
      if (!reg.test(value)) {
        message = name + ' không đúng định dạng (Họ tên không bao gồm số và kí tự đặc biệt)'
      }
    }
...
const mapStateToProps = (rootReducer) => {
  return {
    thongTinSV: rootReducer.quanlySVReducer.thongTinSinhVien,
